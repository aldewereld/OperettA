...
public class DashboardActionRegistry implements IExtensionChangeHandler {
	...
	private Set<AliveDashboardMediator> mediators;

	private Set<DashboardActionDescriptor> descriptors;

	public DashboardActionRegistry() {
		mediators = new HashSet<AliveDashboardMediator>();
	...
	void registerMediator(AliveDashboardMediator mediator) {
		mediators.add(mediator);
	}

	void unregisterMediator(AliveDashboardMediator mediator) {
	...
	public void removeExtension(IExtension extension, Object[] objects) {
		for (Object object : objects) {
			if (object instanceof DashboardActionDescriptor) {
				...
				for (AliveDashboardMediator mediator : mediators) {
	...
	public static class DashboardActionDescriptor {
		...
		public InternalDashboardAction<AliveDashboardState> createDashboardAction() {
			return new Proxy();
		}

		public InternalDashboardAction<AliveDashboardState> createContributedDashboardAction() {
			try {
				return (InternalDashboardAction<AliveDashboardState>) element.createExecutableExtension("class"); //$NON-NLS-1$
			} catch (Exception e) {
		...
		private class Proxy extends InternalDashboardAction<AliveDashboardState> {

			private DashboardFacade<AliveDashboardState> context;

			private boolean inited;

			private InternalDashboardAction<AliveDashboardState> delegate;

			private boolean notAvailable;

			public void init(DashboardFacade<AliveDashboardState> context) {
