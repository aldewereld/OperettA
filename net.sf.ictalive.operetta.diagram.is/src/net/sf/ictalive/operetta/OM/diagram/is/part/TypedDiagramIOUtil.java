...
public class TypedDiagramIOUtil {
	...
	static public Diagram load(final TransactionalEditingDomain domain, String diagramType, final IFile file, boolean bTryCompatible, IProgressMonitor monitor) throws CoreException {
		FileLoader loader = new FileLoader(file);
		return load(domain, diagramType, loader, bTryCompatible, monitor);
	}
	
	static public Diagram load(final TransactionalEditingDomain domain, String diagramType, final IStorage storage, boolean bTryCompatible, IProgressMonitor monitor) throws CoreException {
		ILoader loader = null;
	 ...
	 * 
	 * PATCHED by Martijn Schrage
	 * 
	 * Copied the class from org.eclipse.gmf.runtime.diagram.ui.resources.editor.internal.util.DiagramIOUtil and added extra
	 * diagramType argument to the load methods.
	 * 
	 * @param file
	 * @return
	 * @throws CoreException
	 */
	static private Diagram load(final TransactionalEditingDomain domain, String diagramType, final ILoader loader, boolean bTryCompatible, IProgressMonitor monitor) throws CoreException  {
		...
		try {
			...
			while(rootContents.hasNext()) {
				...
				if(rootElement instanceof Diagram && ((Diagram)rootElement).getDiagram().getType()==diagramType)
					return (Diagram)rootElement;
			}
			
			throw new RuntimeException(NO_DIAGRAM_IN_RESOURCE);
		} catch(Exception e) {
			Trace.catching(EditorPlugin.getInstance(), EditorDebugOptions.EXCEPTIONS_CATCHING, TypedDiagramIOUtil.class, "load(IFile, boolean)", e); //$NON-NLS-1$
			...
			Trace.throwing(EditorPlugin.getInstance(), EditorDebugOptions.EXCEPTIONS_THROWING, TypedDiagramIOUtil.class, "load(IFile, boolean)", thrownExcp); //$NON-NLS-1$
			throw thrownExcp;
		}
	}
